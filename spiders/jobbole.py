...
class JobboleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        """
        1. 获取文章列表的url并交给scrapy进行下载后并进行解析
        2. 获取下一页的url并交给scrapy进行下载，下载完成后交给parse
        :param response: 
        :return: 
        """
        #解析列表页中的所有文章url，并交给scrapy进行下载后并进行解析
        ...
        for post_node in post_nodes:
            ...
            #yield Request(url=parse.urljoin(response.url, image_url), callback=self.parse_details)
        ...
        #for post_url in post_urls:
            ...
            #print (post_url)
        #提取下一页并交给scrapy进行下载
        next_url =  response.css(".next.page-numbers").extract_first("")
        if next_url:
    ...
    def parse_details(self, response):
        ...
        #提取文章的具体字段
        #re1_selector = response.xpath("/html/body/div[3]/div[3]/div[1]/div[1]/text()")
        #re2_selector = response.xpath('//*[@id="post-110287"]/div[1]/h1/text()')

        front_image_url = response.meta.get("front_image_url", "")#文章封面图
        title = response.xpath('//div[@class="entry-header"]/h1/text()').extract()[0]
        create_date  = response.xpath('//p[@class="entry-meta-hide-on-mobile"]/text()').extract()[0].strip().replace("·", "").strip()
        #vote_up = int(response.xpath("//h10[@id='110287votetotal']/text()").extract()[0])
